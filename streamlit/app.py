...
def load_symptoms_model():
    ...
    class CustomAdam(tf.keras.optimizers.Adam):
        def __init__(self, *args, **kwargs):
            ...
            for param in ['use_ema', 'ema_momentum', 'ema_overwrite_frequency', 'jit_compile', 'is_legacy_optimizer']:
                if param in kwargs:
                    del kwargs[param]
...
# Symptoms mapping dictionary (dari kode yang Anda berikan)
symptoms_mapping = {
    'gatal': 'itching',
...
def symptoms_analysis_page():
    ...
    if st.button("Analisis Penyakit"):
        if len(st.session_state.selected_symptoms) > 0:
            # Konversi gejala ke format input model
            ...
            # Reshape input dan lakukan prediksi
...
def predict_multiple_diseases(symptoms_input, threshold=0.1):
    # Implementasi fungsi prediksi sesuai dengan kode yang Anda berikan
    # Referensi ke kode symptoms-predict.ipynb lines 412-458
    pass
...
def disease_prediction_page():
    ...
    with col2:
        blood_pressure = st.number_input('Tekanan Darah Sistolik (mmHg)', 
                                       min_value=0, 
                                       max_value=300, 
                                       value=95,
                                       step=1)
        
        cholesterol = st.number_input('Kolesterol Total (mg/dL)', 
                                    min_value=0, 
                                    max_value=1000, 
                                    value=170,
                                    step=1)
        
        blood_glucose = st.number_input('Gula Darah Puasa (mg/dL)', 
                                      min_value=0, 
                                      max_value=1000, 
                                      value=80,
                                      step=1)
    ...
    if st.button('Analisis Kesehatan'):
        ...
        with col2:
            ...
            # Filter penyakit dengan risiko > 50%
            high_risk_diseases = []
            for disease, prob in zip(diseases, predictions[0]):
                if prob > 0.5:  # 50%
                    risk_level = "Tinggi" if prob < 0.6 else "Sangat Tinggi"
                    high_risk_diseases.append(f"{disease}")
            
            if high_risk_diseases:
                for disease_risk in high_risk_diseases:
                    st.write(disease_risk)
            else:
                st.write("Tidak ada risiko penyakit yang signifikan")

def food_detection_page():
    st.title('Deteksi Makanan dari Gambar')
    # Tampilkan pesan tentang repositori GitHub
    st.write("Untuk informasi lebih lanjut tentang cara menggunakan model deteksi makanan, "
